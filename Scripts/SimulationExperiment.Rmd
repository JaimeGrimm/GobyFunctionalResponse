...
for (i in 1:length(Ex1SimDat)){
  ...
  for (j in 1:1000){
    SimDat <- Ex1SimDat[[i]][m:n, ]
    Output.II <- frair_fit(Ne ~ N0, data = SimDat, response = 'rogersII',
                           start = list(a = 1, h = 0.1), fixed = list(T = 2))
    Output.III <- frair_fit(Ne ~ N0, data = SimDat, response = 'hassIIInr',
    ...
    Output <- if (AIC.Out$dAICc[1] == 0 & (AIC.Out$dAICc[1] + 2) <= AIC.Out$dAICc[2]) {
      "type-II response"
...
Ex1.Prop <- c() 
for (i in 1:length(Ex1SimDat)){
  II.III.NA <- c(length(which(Out.List.Outer[[i]] == "type-II response")),
