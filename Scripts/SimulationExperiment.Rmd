...
```{r}
library(frair); library(lamW); library(tidyverse)
```
...
## SAVING OUTPUTS
saveRDS(Ex1.Prop, "SimOutputs/Ex1Prop_Juliano.rds")
saveRDS(Ex2.Prop, "SimOutputs/Ex2Prop_Juliano.rds")
saveRDS(Ex3.Prop, "SimOutputs/Ex3Prop_Juliano.rds")
```
...
Ex1.Prop <- c() 
for (i in 1:length(Ex1SimDat)){
  Temp <- c(lapply(Out.List.Outer[[i]], getType.GenFR))
  II.III <- c(length(which(Temp == "type-II response")),
                 length(which(Temp == "type-III response")))
  Ex1.Prop <- rbind(Ex1.Prop, II.III)
}
colnames(Ex1.Prop) <- c("II", "III")
rownames(Ex1.Prop) <- c("TLow.II.Unif", "TLow.II.Norm",
                        "TMed.II.Unif", "TMed.II.Norm", "TMed.III.Unif", "TMed.III.Norm",
                        "THigh.II.Unif", "THigh.II.Norm", "THigh.III.Unif", "THigh.III.Norm")
```
